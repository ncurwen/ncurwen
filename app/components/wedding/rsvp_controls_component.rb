...
module Wedding
  class RsvpControlsComponent < ApplicationComponent
    ...
    def button_class(status)
      ...
      # `normal-case` used to sit here for daisyUI 4, which uppercased .btn. daisyUI 5
      # does not, so it was doing nothing.
      base = "btn btn-sm sm:btn-md w-full gap-2"
      return "#{base} btn-primary wedding-answered" if status == "yes" && guest.yes?
      return "#{base} btn-neutral wedding-answered" if status == "no" && guest.no?
